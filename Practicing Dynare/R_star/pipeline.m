...
% %% Wickselian and Taylor comparison with benchamark and full sample:
% 
...
% plot(time_periods, (re_data_taylor-mean(re_data_taylor))*400, 'r-.', 'LineWidth', 2.0, 'DisplayName', 'T')
% hold on
% plot(time_periods, (re_data_wicksellian-mean(re_data_wicksellian))*400, 'g--', 'LineWidth', 2.0, 'DisplayName', 'W')
% hold on
% plot(time_periods, iras-mean(iras), 'k-', 'LineWidth', 1.5, 'DisplayName', 'FFR') % Blue solid line with label "FFR"
% hold on
...
%         plot_name = 'efficient_rate_fullmsample_W_T.png';
...
%         ylim([-12 8])
%         plot_folder = 'figures_results';
%         plot_name = 'efficient_rate_benchmark_W_T.png';
%         plot_path = fullfile(plot_folder, plot_name);
%         saveas(gcf, plot_path);
%         disp(['Plot saved as: ', plot_path]);
%     otherwise
...
%% Convergence back to steady state:

dynare r_star_taylor.mod

re_data_taylor = oo_.SmoothedVariables.re;
forecast_re_taylor = oo_.forecast.Mean.re;
smoothed_re_taylor = oo_.SmoothedVariables.re;

% Concatenate forecast_re and smoothed_re into path_re
path_re_taylor = [smoothed_re_taylor; forecast_re_taylor];

dynare r_star_wicksellian.mod

re_data_wicksellian = oo_.SmoothedVariables.re;
forecast_re_wicksellian = oo_.forecast.Mean.re;
smoothed_re_wicksellian = oo_.SmoothedVariables.re;

% Concatenate forecast_re and smoothed_re into path_re
path_re_wicksellian = [smoothed_re_wicksellian; forecast_re_wicksellian];
...
fig = figure;
fig.Color = 'white'; % Set background color to white
plot(time_periods, (path_re_taylor-mean(path_re_taylor))*400, 'r-.', 'LineWidth', 2.0, 'DisplayName', 'T')
hold on
plot(time_periods, (path_re_wicksellian-mean(path_re_wicksellian))*400, 'g--', 'LineWidth', 2.0, 'DisplayName', 'W')
hold on
%plot(time_periods, iras-mean(iras), 'k-', 'LineWidth', 1.5, 'DisplayName', 'FFR') % Blue solid line with label "FFR"
%hold on
plot(time_periods, 0*path_re_taylor, 'LineWidth', 0.5, 'Color', 'black', 'HandleVisibility', 'off') % Black zero line without legend entry

% Set x-axis limits to start from 2019 Q4
zoom_date = datetime(2019, 07, 1);
xlim([zoom_date end_date]);
tick_positions = zoom_date:calquarters(5):end_date;
...
legend('Location', 'southwest');
xlabel('Time');
ylabel('Efficient real rates');


plot_folder = 'figures_results';
plot_name = 'back_to_steady_state_zoom_W_and_T.png';
plot_path = fullfile(plot_folder, plot_name);
saveas(gcf, plot_path);
disp(['Plot saved as: ', plot_path]);
